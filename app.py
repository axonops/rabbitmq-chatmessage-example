...
def create_response_queue(request_id: str):
    try:
        ...
        logger.error(f"Creation of response queue for request_id {request_id} @ {rabbitmq_url} failed: {e}")
...
def send_message_to_request_queue(request_id: str, message):
    try:
        ...
        logger.info(f"Published message for request_id {request_id} to request queue {request_queue_name}")

        # Close the connection
        connection.close()

    except Exception as e:
        logger.error(f"Error encountered publishing message for request_id {request_id} to request queue {request_queue_name} @ {rabbitmq_url} : {e}")
        raise HTTPException(status_code=503, detail=f"A problem has been sending request {request_id} for processing")
...
async def start_stream(text: str = Form(...)):
    ...
    response_queue_name = create_response_queue(request_id)
    logger.info(f"Created unique response queue {response_queue_name} for request_id {request_id}")
    logger.info("Sending message to request_queue")
    send_message_to_request_queue(request_id, message)
